...
def extract_final_answer_from_content(content: str) -> str:
    """
    Extract the final numeric answer from the content after </think>
    """
    if not content:
        return None
    
    # Look for patterns in the final content (after thinking)
    patterns = [
        r'####\s*(\d+(?:\.\d+)?)',  # Matches "#### 18"
        r'ANSWER:\s*(\d+(?:\.\d+)?)',  # Matches "ANSWER: 18"
        r'Answer:\s*(\d+(?:\.\d+)?)',  # Matches "Answer: 18"
        r'answer:\s*(\d+(?:\.\d+)?)',  # Matches "answer: 18"
        r'Final answer:\s*(\d+(?:\.\d+)?)',  # Matches "Final answer: 18"
        r'final answer:\s*(\d+(?:\.\d+)?)',  # Matches "final answer: 18"
        r'The answer is\s*(\d+(?:\.\d+)?)',  # Matches "The answer is 18"
        r'the answer is\s*(\d+(?:\.\d+)?)',  # Matches "the answer is 18"
        r'[\$£€¥]?\s*(\d+(?:\.\d+)?)\s*(?:dollars?|USD|EUR|GBP|¥)?',  # Matches "$18" or "18 dollars"
    ]
    ...
    # If no pattern found, look for the last number in the content
    numbers = re.findall(r'\b\d+(?:\.\d+)?\b', content_clean)
    if numbers:
        return numbers[-1]
...
def extract_ground_truth(answer_text: str) -> str:
    """
    Extract the ground truth answer from the answer column.
    The format is: reasoning steps... #### final_answer
    """
    # Look for pattern like "#### 18"
    match = re.search(r'####\s*(\d+(?:\.\d+)?)', answer_text)
    if match:
        return match.group(1)
    
    # If no #### pattern, try to extract the last number
...
def validate_parquet_structure(file_path: str) -> Tuple[bool, List[str], pd.DataFrame]:
    """
    Validate the parquet file structure and return sample data
    """
    try:
        ...
        # Check for expected columns (using lowercase)
        ...
        # Check answer format
        answer_sample = df['answer'].iloc[0]
        print(f"📝 Answer format sample: '{answer_sample}'")
        
        # Extract ground truth from sample
        ground_truth = extract_ground_truth(answer_sample)
        print(f"🎯 Extracted ground truth: '{ground_truth}'")
...
Please think through this problem step by step and provide your reasoning. Then, give your final answer in the format: "#### X" where X is your final numeric answer."""
...
def create_csv_logger(filename: str):
    ...
    headers = [
        ...
        'thinking_content_preview', 'final_content_preview'
    ]
...
def benchmark_model():
    """Benchmark the model on the parquet dataset with comprehensive CSV logging"""
    ...
    print("🧪 MODEL BENCHMARKING WITH PARQUET DATASET")
    print("=" * 60)
    print(f"📁 Results will be saved to: {csv_filename}")
    
    # Validate parquet file
    print("📁 Validating parquet file...")
    is_valid, columns, sample_data = validate_parquet_structure(parquet_path)
    
    if not is_valid:
        print("❌ Invalid parquet file structure. Please check the file.")
        return
    ...
    for idx, (_, row) in enumerate(df.head(max_samples).iterrows()):
        ...
        if not ground_truth:
            print(f"❌ Could not extract ground truth from: {answer_text}")
            continue
        ...
        # CORRECTED: Parse thinking content using the official method
        decode_start = time.time()  # FIXED: Added missing decode_start
        ...
        # CORRECTED: Extract final answer only from the final content (after </think>)
        predicted_answer = extract_final_answer_from_content(final_content)
        
        # Check if correct (numeric comparison)
        is_correct = False
        if predicted_answer and ground_truth:
            try:
                ...
                is_correct = abs(pred_num - truth_num) < 0.001  # Allow small floating point differences
            except ValueError:
        ...
        print(f"🎯 Predicted: {predicted_answer} | Ground Truth: {ground_truth} | ✅ Correct: {is_correct}")
    ...
    if results:
        summary_filename = os.path.join(results_dir, f"benchmark_summary_{timestamp}.txt")
        with open(summary_filename, 'w') as f:
            f.write("BENCHMARK SUMMARY\n")
            f.write("================\n\n")
            f.write(f"Model: {os.path.basename(model_path)}\n")
            f.write(f"Dataset: {os.path.basename(parquet_path)}\n")
